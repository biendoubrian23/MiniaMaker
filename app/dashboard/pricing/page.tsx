        ...
        <div className="p-8 min-h-screen">
            ...
            <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                {PLANS.map((plan) => (
                    <div
                        key={plan.id}
                        className={`
              relative bg-white border-4 p-8
              ${plan.popular
                                ? 'border-youtubeRed shadow-[8px_8px_0px_0px_rgba(255,0,0,1)]'
            ...
            `}
                    >
                        ...
                        <div className="mb-6 p-4 bg-gray-50 border-2 border-gray-200">
                            <div className="text-3xl font-bold text-youtubeRed">{plan.credits}</div>
                        ...
                        <ul className="mb-8 space-y-3">
                            <li className="flex items-center gap-2">
                                ...
                                <span>{plan.credits} {t('pricing.feature1')}</span>
                            </li>
                            <li className="flex items-center gap-2">
                                <span className="text-green-500">✓</span>
                        ...
                        {/* Bouton */}
                        <a
                            href={getStripeUrl(plan.stripeLink)}
                            className={`
                block w-full text-center px-6 py-4 font-bold text-lg border-4 border-black transition-all duration-200
                ${plan.popular
                                    ? 'bg-youtubeRed text-white hover:bg-red-600'
            ...
            <div className="max-w-4xl mx-auto mt-16">
                ...
                <div className="space-y-4">
                    {[1, 2, 3].map((num) => (
                        <div key={num} className="bg-white border-2 border-black p-6">
                            <h3 className="font-bold text-lg mb-2">{t(`pricing.faq${num}Q`)}</h3>
                            <p className="text-gray-600">{t(`pricing.faq${num}A`)}</p>
                        </div>
                    ))}
                </div>
            </div>
